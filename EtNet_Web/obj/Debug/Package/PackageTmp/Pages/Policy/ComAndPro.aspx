<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ComAndPro.aspx.cs" Inherits="EtNet_Web.Pages.Policy.ComAndPro" %>
...
<head runat="server">
    ...
    <style type="text/css">
        ...
        h3
        {
            background: url("../../images/public/bg.png") no-repeat rgb(255, 255, 255);
            font-weight: normal;
    ...
    <script type="text/javascript">
        ...
        function zTreeOnClick(event, treeId, treeNode) {
            $('#HidTypeId').val(treeNode.id);
            $('#HidTypeName').val(treeNode.name);
        ...
        function LoadTree(id, name) {
            $('#HidTypeName').val("");
            $('#HidTypeId').val("");
            $.ajax({
                type: "get",
                url: "LoadTreeHandler.ashx",
                dataType: "text",
                data: { 'id': id },
        ...
        function Save() {
            ...
            else if ($('#HidTypeId').val() == "") {
                alert("请选择险种");
                return;
            }
            var origin = artDialog.open.origin;
            var company = origin.document.getElementById('TxtCompany');
            var type = origin.document.getElementById('TxtType');
            var comid = origin.document.getElementById('HidComId');
            var typeid = origin.document.getElementById('HidTypeId');
            company.value = $('#HidComName').val();
            type.value = $('#HidTypeName').val();
            comid.value = $('#HidComId').val();
            typeid.value = $('#HidTypeId').val();

            window.parent.getTarget();
//            window.parent.GetItem($('#HidTypeId').val());
            art.dialog.close();
        }
...
<body>
        ...
        <div id="left">
            ...
            <div class="box_r">
                <asp:Repeater ID="RpCompany" runat="server">
                    ...
                    <ItemTemplate>
                        <li><a href="javascript:void(0);" onclick='LoadTree(<%# "\""+ Eval("id").ToString() +"\",\""+ Eval("comCname").ToString()+"\"" %>)'>
                            <%# Eval("comCname")%></a></li>
                    </ItemTemplate>
                    <FooterTemplate>
                        </ul>
    ...
    <input id="HidTypeName" type="hidden" />
