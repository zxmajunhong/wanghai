<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AusTypeList.aspx.cs" Inherits="EtNet_Web.Pages.AusType.AusTypeList" %>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#addtxt").click(function () {
                window.location = "AddAusType.aspx";
            })
            ...
            $("#editpage").click(function () {
                ...
                strmodal += "status=no";
                window.showModalDialog("../Common/PageSearchSet.aspx?pagenum=023&dt=" + new Date().toString(), window.self, strmodal);
            })
                ...
                <td style="vertical-align: top;">
                    <table class="clsdata" cellspacing="1" cellpadding="0">
                        <tr>
                            ...
                            <th class="clstitleimg" style="width: 80%;">
                                类型名称
                            </th>
                            ...
                            <asp:Repeater runat="server" ID="cuslist" OnItemCommand="cuslist_ItemCommand">
                                <ItemTemplate>
                                    <tr>
                                        ...
                                        <td>
                                            <%--<asp:ImageButton ID="cy" title="常用" AlternateText="常用" runat="server" CommandName="Tick" CommandArgument='<%# Eval("id") %>'
                                            ImageUrl="~/Images/icons/tick.png" Visible='<%# isCy1(Eval("id").ToString()) %>' />
                                            <asp:ImageButton ID="bcy" title="不常用" AlternateText="不常用" runat="server" CommandName="TurnBack" CommandArgument='<%# Eval("id") %>'
                                            ImageUrl="~/Images/icons/turnback.png" Visible='<%# isCy2(Eval("id").ToString()) %>' />--%>
                                            <asp:Label ID="lblcy" runat="server" Style="color:Green" Text="常用" Visible='<%# isCy2(Eval("id").ToString()) %>'></asp:Label>
                                            <asp:Label ID="lblbcy" runat="server" Style="color:Red" Text="不常用" Visible='<%# isCy1(Eval("id").ToString()) %>'></asp:Label>
                                        </td>
                                        <td>
                                            <asp:ImageButton ID="ImageButton3" title="编辑" AlternateText="编辑" runat="server" CommandName="Update"
