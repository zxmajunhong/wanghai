...
function select() {
    with (document.getElementById('myform').sheng) { var loca2 = options[selectedIndex].value; }
    for (i = 0; i < where.length; i++) {
        if (where[i].loca == loca2) {
            loca3 = (where[i].locashi).split("|");
            for (j = 0; j < loca3.length; j++) {
                with (document.getElementById('myform').shi) {
                    length = loca3.length;
...
loca4 = options[selectedIndex].value;
    ...
    document.getElementById('myform').address.value = loca2 + " " + loca4;
}
function init() {
    debugger;
    with (document.getElementById('myform').sheng) {
        length = where.length;
