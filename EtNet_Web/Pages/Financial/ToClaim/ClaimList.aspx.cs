    ...
    public partial class ClaimList : System.Web.UI.Page
    {
        ...
        private void FilterReceipt()
        {
            ...
            if (txtremark.Text.Trim()!=string.Empty)
                filterBuilder.AppendFormat(" AND receiptMark LIKE '%{0}%' ", txtremark.Text.Trim());
            ...
            if (ddlClaimStatus.SelectedIndex != 0)
                filterBuilder.AppendFormat(" AND receiptStatusCode = {0} ", ddlClaimStatus.SelectedValue);
        ...
        private void SendMessage(int collectingID, int falg)
        {
            ...
            string salesman = ((LoginInfo)Session["login"]).Cname;
