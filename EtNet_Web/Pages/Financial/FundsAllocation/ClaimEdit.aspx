<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ClaimEdit.aspx.cs" Inherits="EtNet_Web.Pages.Financial.FundAllocation.ClaimEdit" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<%--2013年1月7日15:30:06--%>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title></title><script src="../../../Scripts/jquery-1.7.2.min.js" type="text/javascript"></script>
    ...
    <script type="text/javascript">
        function selectPolicy() {
            ...
            art.artDialog.open('SelectPolicy.aspx?salesman=' + salesmanId + "&payer=" + payer + "&payertype=" + payerType + "&type=" + type, { width: '600px', height: '400px' }).lock().title('选择保单');
        }
        $(document).ready
        (
            function () {
                ...
                $("#chkBox,#chkFinish").click(function () {
                    ...
                    $(".mytable .num").each(function () {
                        toatl += parseFloat($.trim($(this).val()).replace("￥", "").replace("¥", ""));
                    });
                    if (isNaN(toatl)) {
                        alert("输入金额不正确");
                        $("#chkFinish").removeAttr("checked");
        ...
        function readData() {
            if (Validator.Validate(document.getElementById('form1'), 1)) {
                ...
                $(".mytable tbody tr:has(input)").each(function () {
                    ...
                    toatl += parseFloat($.trim($(this).find("input").val()).replace("￥", "").replace("¥", ""));
                });
                if (toatl > amount) {
                    alert("输入金额总数大于收款金额");
                    return false;
    ...
    <div class="border">
        <span style="display: block; width: 100%; text-align: right;">
            <asp:ImageButton ID="BtnSubmit" OnClientClick="return readData();"
                runat="server" onclick="BtnSubmit_Click" ImageUrl="~/Images/button/btn_save.jpg" ToolTip="保存" />
        ...
        <div style="border:1px solid #CDC9C9;margin-top:20px;padding:10px;">
            <table id="Table1" style="text-align: center;" cellspacing="1" class="mytable">
                <caption>
                    上次选择保单
                </caption>
                <thead>
                ...
                <tbody id="Tbody1">
                    <asp:Repeater ID="RpPolicyList" runat="server">
                        <ItemTemplate>
                            <tr>
                                <td class="del"><a href="javascript:void(0);" onclick='delRow(this);'><img src='../../../images/public/filedelete.gif'/></a></td>
                                <td><%# Eval("serialnum")%></td>
                                <td><%# Eval("Policy_Num")%></td>
                                <td class="amount"><%# Eval("claimAmount","{0:F2}") %></td>
                                <td><input class="num" readonly="readonly" value='<%# Eval("realAmount","{0:F2}") %>'</td>
        ...
        <div style="border:1px solid #CDC9C9;margin-top:20px;padding:10px;text-align:right;vertical-align:middle;">
            <span id="chkBox" style="cursor:pointer;"><input id="chkFinish" runat="server" type="checkbox" style="vertical-align:middle;" />确认完成收款？</span>
        </div>
    </div>
