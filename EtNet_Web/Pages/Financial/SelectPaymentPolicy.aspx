<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SelectPaymentPolicy.aspx.cs"
    Inherits="EtNet_Web.Pages.Financial.SelectPaymentPolicy" %>
                ...
                <asp:Repeater ID="RpPolicyList" runat="server">
                    <ItemTemplate>
                        <tr>
                            ...
                            <td class="policyAmount">
                                <%# Eval("policyAmount") == DBNull.Value ? "0.00" : Eval("policyAmount").ToString() %>
                            </td>
                            <td>
                                <input style="color: Red;" class="amount" datatype="Require" msg="金额不能为空" type="text"
                                    readonly="readonly" maxvalue='<%# GetAmount(Eval("policyAmount"),Eval("payAmount")) %>'
                                    value='<%# GetAmount(Eval("policyAmount"),Eval("payAmount")) %>' />
                            </td>
                            <td class="nouse" style="color: Green;">
                                <%# Eval("payAmount") == DBNull.Value ? "0.00" : Eval("payAmount").ToString() %>
                            </td>
                            <td class="pid" style="display: none; width: 0px;">
                                <%# Eval("policyID") %>
                            </td>
                            <td class="pnum" style="display: none; width: 0px;">
                                <%# Eval("policy_num")%>
                            </td>
                            <td class="psnum" style="display: none; width: 0px;">
                                <%# Eval("serialnum")%>
                            </td>
                            <td class="invoiceNum" style="display: none; width: 0px;">
                            </td>
                        </tr>
                    </ItemTemplate>
                </asp:Repeater>
